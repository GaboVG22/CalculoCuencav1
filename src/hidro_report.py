...
def generate_memoria_docx(
    ...
    doc.add_paragraph(
        ...
        f"{H:.2f} m, equivalente a una pendiente media de {S:.2f} %. El tiempo de concentración Kirpich "
        f"preliminar es {Tc:.2f} minutos."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "Los métodos DGA-AC y Verni-King se implementan en esta versión como módulos parametrizados: la aplicación permite calcularlos "
        "al ingresar los coeficientes oficiales o adoptados por el especialista, evitando usar constantes no verificadas."
    )
    if notes:
